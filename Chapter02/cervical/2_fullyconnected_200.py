...
def reformat(dataset, labels):
  ...
  #dataset = dataset.flatten().astype(np.float32)
...
def accuracy(predictions, labels):
  ...
  b = np.argmax(predictions, 1) == np.argmax(labels, 1)
  c = np.sum(np.argmax(predictions, 1) == np.argmax(labels, 1))
  r = (100.0 * np.sum(np.argmax(predictions, 1) == np.argmax(labels, 1))
          / predictions.shape[0])
